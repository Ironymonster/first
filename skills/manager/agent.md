...
### 步骤 0.25：确认 req-id（防止复用已有编号）

**在创建分支之前，必须先确认本次需求使用的 req-id 是全新编号，不能复用任何已存在的目录**
...
```bash
git checkout -b feat/req-<id> origin/master
```
...
## 阶段 0.5：初始化开发规范 (Rules Init)

**在阶段 0 完成、阶段 1 需求沟通之前执行一次。每个项目只需初始化一次**
...
Spec Agent 完成后，更新 `docs/index.json`，将本次需求追加到 `requirements` 列表。

如果 `docs/index.json` 不存在，先创建（见本文件末尾「`docs/index.json` 维护规范」章节）。
...
并让用户确认 Demo 是否符合基本要求，符合才进入实际的开发，否则退回到步骤 1.1 需求沟通，重新沟通后再次 spawn @spec 更新 artifacts，再重新生成 Demo。
...
```
  ...
  3. 修复完成后，重新跳回阶段 3 执行测试。最多重试 10 轮。

## 阶段 4.5：代码质量优化 (Code Refinement)
...
```json
{
  ...
  "req_id": "<id>",
  "target": "frontend",
  "agent_result": {"status": "ok", "exit_code": 0, "elapsed_seconds": 90.0}
}
```
...
### 步骤 4.5.3：更新开发规范（updateRule）

**根据本次代码优化和修复阶段发现的 bug 模式、性能优化经验，更新开发规范文件（mdc + template 同步维护）**

收集本次需求中的以下经验：

- 阶段 4 修复的 bug 模式（来自测试报告中的 Bug 汇总）
- 阶段 4.5 代码优化中发现的性能问题（来自 frontend-report.md / backend-report.md 的「代码优化」章节）
...
### 步骤 4.5.4：优化完成后进入阶段 5

代码优化和规范更新完成后，直接进入 **阶段 5：完成汇报**。
...
- **阶段 0**：`git pull origin master` 同步 → 检查未合并分支 → `git checkout -b feat/req-<id>` 创建特性分支
- **阶段 1-4**：每个 orchestrator 命令带 `--git-commit` 自动提交变更
...
```bash
# 查看已有的 fix 分支编号，取最大值 + 1
git branch -r | grep 'origin/fix/bug-' | sed 's/.*bug-//' | sort -n | tail -1
```

规则：若无任何 `fix/bug-*` 分支，从 bug-001 开始；否则最大编号 + 1。

### 步骤 B0.3：创建修复分支

```bash
git checkout -b fix/bug-<seq> origin/master
```
...
```bash
chainagent bugfix --agent frontend --description "BUG-<seq>: <根因描述> | 修复方向: <修复方案> | 涉及文件: <文件列表>" --git-commit
```
...
```bash
chainagent bugfix --agent backend --description "BUG-<seq>: <根因描述> | 修复方向: <修复方案> | 涉及文件: <文件列表>" --git-commit
```
...
修复完成后，spawn Test Agent 针对本次 bug 进行专项验证。

```
task(subagent_type="test", run_in_background=false, prompt="
## 任务指令
对 BUG-<seq> 的修复结果进行专项验证。
## Bug 信息
- Bug 编号：BUG-<seq>
- Bug 描述：<一句话描述>
- 修复报告：reports/fix-reports/BUG-<seq>-*-fix.md
## 验证范围
1. 验证 bug 已修复：按复现步骤执行，确认异常行为不再出现
2. 回归验证：检查修复是否影响相邻功能（重点验证改动文件的相关逻辑）
3. 后端测试（如涉及）：cd backend && go test ./... -v
4. 前端编译检查（如涉及）：cd frontend && npm run build
## 完成后
- 生成测试报告 reports/bug-test-report-<seq>.md，包含：验证结论（PASS/FAIL）、测试明细、回归影响评估
- 如测试通过（PASS），在报告末尾注明「修复验证通过，可合并」
- 如测试失败（FAIL），在报告末尾列出仍存在的问题，由 Manager 决策
- 执行：git add -A && git commit -m '[BUG-<seq>] test: Verification report'
")
```
...
| 时机 | 操作 |
|---|---|
| 步骤 1.25（策划完成后） | `requirements` 新增条目，status=`planning` |
| 阶段 2 开始时 | 更新对应 REQ 的 status=`developing` |
| 阶段 3 开始时 | 更新对应 REQ 的 status=`testing` |
| 阶段 4 修复时 | 更新 REQ status=`fixing`；`bugs` 新增条目（每个 bug 一条） |
| 阶段 5 完成后 | 更新 REQ status=`done`，updated_at=当前日期；bug status=`fixed` |
| Bug 流水线 B1 发现时 | `bugs` 新增条目，status=`open` |
| Bug 流水线 B2 修复完成 | 更新 bug status=`fixed` |
